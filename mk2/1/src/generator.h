 ...
 *
 * Any exception will be propagated to the user by begin() or operator++() functions.
 */
template <typename T>
struct Generator {
...
struct Generator<T>::GeneratorPromiseType {
    ...
    Expression&& await_transform(Expression&& expression) {
        static_assert(
            sizeof(expression) == 0, "co_await is not supported in coroutines of type generator");
