...
int print_message(message_t *message) {
  std::cout << "Message: \"" << message->sender << ": "
	                           << message->data << "\"\n";
  return 0;
}
...
int create_message_queue(int queue_size, message_queue_t *queue) {
  queue->data = reinterpret_cast<message_t*>(
		calloc(queue_size, sizeof(message_t))
	);
...
typedef struct {
   ...
   * 
   * In case we don't send whole message per one call send().
...
int receive_from_peer(peer_t *peer, int (*message_handler)(message_t *)) {
  std::cout << "Ready for recv() from " << peer_get_addres_str(peer) << ".\n";
  ...
  do {
    ...
    std::cout << "Let's try to recv() " << len_to_receive << " bytes... ";
    received_count = recv(
			peer->socket,
    ...
    if (received_count < 0) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        std::cout << "peer is not ready right now, try again later.\n";
      } else {
        perror("recv() from peer error");
        return -1;
      }
    } else if (
			received_count < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)
		) {
      break;
    } else if (received_count == 0) {
    	// If recv() returns 0, it means that peer gracefully shutdown.
			// Shutdown client.
      std::cout << "recv() 0 bytes. Peer gracefully shutdown.\n";
      return -1;
    } else if (received_count > 0) {
      peer->current_receiving_byte += received_count;
      received_total += received_count;
      std::cout << "recv() " << received_count << " bytes\n";
    }
  } while (received_count > 0);
  
  std::cout << "Total recv()'ed " << received_total << " bytes.\n";
  return 0;
}

int send_to_peer(peer_t *peer) {
  std::cout << "Ready for send() to " << peer_get_addres_str(peer) << ".\n";
  
  size_t len_to_send;
  ssize_t send_count;
  size_t send_total = 0;
  do {
    // If sending message has completely sent and there are messages in queue
		// , why not send them?
    if ( peer->current_sending_byte < 0
		  || peer->current_sending_byte >= sizeof(peer->sending_buffer)
		) {
      std::cout << "There is no pending to send() message, " 
			          << "maybe we can find one in queue... ";
      if (dequeue(&peer->send_buffer, &peer->sending_buffer) != 0) {
        peer->current_sending_byte = -1;
        std::cout << "No, there is nothing to send() anymore.\n";
        break;
      }
      std::cout << "Yes, pop and send() one of them.\n";
    ...
    std::cout << "Let's try to send() " << len_to_send << " bytes... ";
    send_count = send(
			peer->socket,
    ...
    if (send_count < 0) {
      ...
      std::cout << "send()'ed 0 bytes. "
			          << "It seems that peer can't accept data right now. "
								<< "Try again later.\n";
      break;
    } else if (send_count > 0) {
      peer->current_sending_byte += send_count;
      send_total += send_count;
      std::cout << "send()'ed " << send_count << " bytes.\n";
    }
  } while (send_count > 0);

  std::cout << "Total send()'ed " << send_total << " bytes.\n";
  return 0;
}
...
// This message enqueues to send queue.
int read_from_stdin(char *read_buffer, size_t max_len) {
  ...
  do {
    ...
    if (read_count < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
      ...
      if (total_read > max_len) {
        std::cout << "Message too large and will be chopped. "
  ...
  std::cout << "Read from stdin " << strlen(read_buffer)
	          << " bytes. Let's prepare message to send.\n";
