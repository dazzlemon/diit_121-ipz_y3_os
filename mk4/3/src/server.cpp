...
void shutdown_properly(int code) {
  ...
  std::cout << "Shutdown server properly.\n";
  exit(code);
}

void handle_signal_action(int sig_number) {
  if (sig_number == SIGINT) {
		std::cout << "SIGINT was catched!\n";
    shutdown_properly(EXIT_SUCCESS);
  } else if (sig_number == SIGPIPE) {
    std::cout << "SIGPIPE was catched!\n";
    shutdown_properly(EXIT_SUCCESS);
  }
}
...
int start_listen_socket(int *listen_sock) {
  ...
  *listen_sock = socket(AF_INET, SOCK_STREAM, 0);
  ...
  int reuse = 1;
  if (setsockopt(
		*listen_sock, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse)
	) != 0) {
    perror("setsockopt");
    return -1;
  }
  ...
  if (bind(
		*listen_sock, (struct sockaddr*)&my_addr, sizeof(struct sockaddr)
	) != 0) {
    perror("bind");
    return -1;
  }
  ...
  std::cout << "Accepting connections on port "
...
int handle_new_connection() {
  ...
  std::cout << "Incoming connection from " << client_ipv4_str << ":"
  ...
  std::cout << "There is too much connections. Close new connection "
...
int close_client_connection(peer_t *client) {
  std::cout << "Close client socket for "
  ...
  for (int i = 0; i < MAX_CLIENTS; ++i) {
    if (connection_list[i].socket != NO_SOCKET) {
      if (peer_add_to_send(&connection_list[i], &new_message) != 0) {
        std::cout << "Send buffer was overflowed, we lost this message!\n";
        continue;
      }
      std::cout << "New message to send was enqueued right now.\n";
    }
  }
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    int activity = select(
			high_sock + 1, &read_fds, &write_fds, &except_fds, NULL
		);
    ...
    switch (activity) {
      ...
      case 0:
        ...
        std::cout << "select() returns 0.\n";
        shutdown_properly(EXIT_FAILURE);
      default:
        /* All set fds should be checked. */
        if ( FD_ISSET(STDIN_FILENO, &read_fds)
				  && handle_read_from_stdin() != 0) {
					shutdown_properly(EXIT_FAILURE);
        }
        ...
        if (FD_ISSET(STDIN_FILENO, &except_fds)) {
          std::cout << "except_fds for stdin.\n";
          shutdown_properly(EXIT_FAILURE);
        }

        if (FD_ISSET(listen_sock, &except_fds)) {
          std::cout << "Exception listen socket fd.\n";
          shutdown_properly(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < MAX_CLIENTS; ++i) {
          if ( connection_list[i].socket != NO_SOCKET 
					  && FD_ISSET(connection_list[i].socket, &read_fds)
						&& receive_from_peer(
							&connection_list[i], &handle_received_message
						) != 0
					) {
          ...
          if ( connection_list[i].socket != NO_SOCKET
					  && FD_ISSET(connection_list[i].socket, &write_fds)
						&& send_to_peer(&connection_list[i]) != 0
					) {
						close_client_connection(&connection_list[i]);
						continue;
          }
    ...
    std::cout << "And we are still waiting for clients' or stdin activity. "
